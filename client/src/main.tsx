...
function App() {
  ...
  const [tab, setTab] = useState<'repair' | 'activity' | 'state'>('repair');
  const [evidence, setEvidence] = useState<RepairOperation>();
  const [interrupt, setInterrupt] = useState(false);
  const [showReset, setShowReset] = useState(false);
  const [config, setConfig] = useState({ investigator: 'scenario', model: '', twins: 'unconfigured', mode: 'local' });
  ...
  const held = plan?.operations.filter(o => o.status === 'held').length || 0;
  ...
  function exportReceipt() {
    if (!w) return;
    const url = URL.createObjectURL(new Blob([JSON.stringify({ mode: w.mode, incidentId: w.incidentId, plan, records: w.records, events: w.events }, null, 2)], { type: 'application/json' }));
    const a = document.createElement('a'); a.href = url; a.download = 'aftercare-recovery-receipt.json'; a.click(); URL.revokeObjectURL(url);
  }
  return <div className="shell">
    <aside className="sidebar">
      ...
      <div className="nav-label">{twinMode ? 'CONNECTED TO TWINS' : 'CONNECTED TO SCENARIO'}</div>
      {(['GitHub', 'Linear', 'Slack'] as AppName[]).map(app => <button className="app-nav" key={app} onClick={() => setTab('state')}><AppIcon app={app} small />{app}<span className="local-dot" style={twinMode ? { background: '#7fae8f' } : undefined} /></button>)}
      <div className="sidebar-bottom">
        <div className="sandbox-label"><Code2 size={15} />{twinMode ? 'ARGA TWINS' : 'LOCAL SCENARIO'}</div>
        <p>{twinMode
        ...
        {!twinMode && config.twins === 'available' && <button className="button subtle full" style={{ marginTop: 12 }} disabled={!!busy} onClick={() => action('provision-twins')}>{busy === 'provision-twins' ? <><LoaderCircle size={13} className="spin" />Provisioning…</> : <><Zap size={13} />Provision twins</>}</button>}
      ...
      <main>
        ...
        {!w ? <div className="loading"><LoaderCircle className="spin" />{error ? 'Workspace unavailable. Reload to try again.' : 'Loading recovery workspace…'}</div> : <>
          <section className="incident-card">
            ...
            <p className="incident-description">A duplicate issue, an incorrect owner, and a premature completion message.<br className="desktop-break" /> Three successful API calls. One unfinished onboarding.</p>
            <div className="incident-meta"><span><span className="agent-avatar"><Sparkles size={12} /></span>onboarding-agent</span><span><Clock3 size={13} />Recorded scenario</span><span className="mono">run_8f24</span><div className="app-stack">{(['GitHub', 'Linear', 'Slack'] as AppName[]).map(app => <AppIcon key={app} app={app} small />)}<span>3 apps affected</span></div></div>
          ...
          <div className="content-grid"><section className="review-panel">
            <div className="tabs" role="tablist" aria-label="Recovery details">{([['repair', 'Repair plan'], ['activity', 'Activity'], ['state', 'App state']] as const).map(([id, label]) => <button key={id} role="tab" aria-selected={tab === id} onClick={() => setTab(id)} className={tab === id ? 'tab active' : 'tab'}>{label}{id === 'repair' && plan && <span>{writes}</span>}{id === 'activity' && <span>{w.events.length}</span>}</button>)}<span className="tab-note">{plan ? `VERSION ${String(plan.version).padStart(2, '0')}` : 'AWAITING INVESTIGATION'}</span></div>
            {tab === 'repair' && <div className="tab-content">
              {!plan ? <div className="empty-plan"><span className="empty-icon"><Layers3 size={28} /></span><h3>Every recovery starts with evidence.</h3><p>Compare the agent’s recorded actions with the current app state to prepare a reviewable repair.</p><button className="button primary" disabled={!!busy} onClick={() => action('prepare')}>{busy ? <LoaderCircle className="spin" size={15} /> : <Sparkles size={15} />}{busy === 'prepare' ? 'Investigating…' : config.investigator === 'openrouter' ? 'Investigate & prepare repair' : 'Prepare repair plan'}<ArrowRight size={15} /></button><small>{config.investigator === 'openrouter' ? 'OpenRouter investigator · read-only tools · no changes applied' : 'Scenario rules · 3 recorded actions · no changes applied'}</small></div> : <>
                <div className="plan-intro"><div><h3>{complete ? 'The outcome, verified.' : stale ? 'The world changed. Your plan should too.' : 'Small corrections. Clear consequences.'}</h3><p>{complete ? `Each approved change was checked against the resulting ${twinMode ? 'twin' : 'local'} app state.` : stale ? 'A human updated a record after this preview. Refresh before approving.' : `${writes} proposed changes${held ? ` · ${held} human decision preserved` : ' · review each change before approval'}.`}</p></div><ShieldCheck size={21} /></div>
                {w.investigation && <div className="investigation-summary"><div><Sparkles size={15} /><span>AI INVESTIGATION</span><small>{w.investigation.toolCalls} tool calls</small></div><p>{w.investigation.summary}</p><small>{w.investigation.model} · recommendations validated by recovery policy</small></div>}
                {plan.operations.map((op, i) => <article className={`repair-card ${op.status === 'held' ? 'held' : ''}`} key={op.id}>
                  ...
                  <div className="field-diff"><div className="diff-label">{op.field === 'correction' ? 'THREAD REPLY' : op.field.toUpperCase()}</div>{op.status === 'held' || op.status === 'unchanged' ? <div className="preserved-value"><LockKeyhole size={13} />{op.observed}<span>Keep current value</span></div> : <><div className="diff-before"><span>−</span>{op.observed || 'No correction posted'}</div><div className="diff-after"><span>+</span>{op.proposed}</div></>}</div>
                  <div className="repair-bottom"><p>{op.reason}</p><button className="evidence-link" onClick={() => setEvidence(op)}>Evidence<ArrowUpRight size={13} /></button></div>
                </article>)}
                <div className="plan-footnote"><LockKeyhole size={13} /><span>{twinMode ? 'Twin state is checked before each write. Providers do not offer atomic protection against edits between a read and write.' : complete ? 'Verification covers this local scenario.' : 'State is checked before each write. A changed record blocks a stale repair.'}</span></div>
              </>}
            </div>}
