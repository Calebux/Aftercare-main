    ...
    <div className="main-shell">
      ...
      <main>
        ...
        {!w ? <div className="loading"><LoaderCircle className="spin" />{error ? 'Workspace unavailable. Reload to try again.' : 'Loading recovery workspace…'}</div> : <>
          ...
          {config.connections === 'enabled' && w.mode === 'local' && !plan && busy !== 'connect-live' && <AgentConnect icon={app => <AppIcon app={app} small />} onRunFinished={() => { fetch('/api/workspace').then(r => r.json()).then((next: Workspace) => { setW(next); if (next.mode === 'live') setTab('run'); }).catch(() => {}); }} />}
          ...
          <section className="incident-card">
            <div className="incident-top"><div className="incident-title"><span className={`incident-icon ${complete ? 'resolved' : ''}`}>{complete ? <CheckCheck size={20} /> : <GitBranch size={20} />}</span><div><div className="small-label">{incidentCopy(w.incident).label}</div><h2>{incidentCopy(w.incident).headline}</h2></div></div><span className={`status-pill ${complete ? 'green' : escalated || stale || interrupted ? 'amber' : 'purple'}`}><span />{escalated ? 'Human investigation required' : complete ? 'Recovery verified' : stale ? 'Review needs updating' : interrupted ? 'Recovery interrupted' : approved ? 'Approved to repair' : plan ? 'Ready for review' : 'Needs recovery'}</span></div>
            <p className="incident-description">{incidentCopy(w.incident).description}<br className="desktop-break" /> {w.run ? `${w.run.agent} made ${w.run.actions.filter(a => a.actor === w.run!.agent).length} changes; ${w.run.actions.filter(a => a.assessment === 'needs_repair').length} flagged for review.` : 'Three successful API calls. One unfinished onboarding.'}</p>
            <div className="incident-meta"><span><span className="agent-avatar"><Sparkles size={12} /></span>{w.run?.agent ?? 'onboarding-agent'}</span><span><Clock3 size={13} />{w.run?.mode === 'recorded' ? 'Recorded from real tool calls' : 'Simulated recording'}</span><span className="mono">run_8f24</span><div className="app-stack">{[...new Set(w.records.map(r => r.app))].map(app => <AppIcon key={app} app={app} small />)}<span>{new Set(w.records.map(r => r.app)).size} apps affected</span></div></div>
          ...
          <aside className="right-column">
            <section className="decision-card"><div className="small-label">RECOVERY SUMMARY</div><h3>{complete ? 'Back on solid ground.' : 'You stay in control.'}</h3><p>{complete ? 'The repair has finished. Review or export the evidence behind the result.' : 'Approve a specific set of changes, with the evidence in reach.'}</p><div className="summary-stats"><div><span>Apps involved</span><strong>{String(new Set(w.records.map(r => r.app)).size).padStart(2, '0')}</strong></div><div><span>{complete ? 'Changes verified' : 'Proposed changes'}</span><strong>{String(complete ? verified : writes).padStart(2, '0')}</strong></div><div><span>Records preserved</span><strong>{String(held).padStart(2, '0')}</strong></div></div>
